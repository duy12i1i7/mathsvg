...
if [ "${MATHZIP_SKIP_DOWNLOAD:-0}" != "1" ]; then
    python3 python/download_datasets.py \
        --config configs/recursive-ablation.yaml \
...
python3 python/verify_results.py \
    benchmarks/results/recursive-ablation/latest.json \
    --strict
python3 python/generate_plots.py \
    benchmarks/results/recursive-ablation/latest.json \
    --output benchmarks/plots/recursive-ablation
python3 python/generate_report.py \
    benchmarks/results/recursive-ablation/latest.json \
    --output benchmarks/results/recursive-ablation/latest-report.md
